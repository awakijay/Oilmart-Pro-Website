...
export const seedProducts: Product[] = [
  ...
  },
  {
    ...
    category: 'Vessels',
    price: formatNaira(2500000),
    image: 'https://images.unsplash.com/photo-1500375592092-40eb2168fd21?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=800',
    rating: 4.8,
    orders: '28+',
    description: 'Multi-purpose offshore support vessel suitable for crew movement, light cargo operations, and offshore field logistics.',
    specifications: {
      'Length Overall': '68 m',
      'Deck Space': '450 m2',
      'Accommodation': '48 POB',
      'Propulsion': 'Twin Diesel',
      'Class': 'Offshore Support Vessel',
    }
  },
];
...
export function getCategories(items: Product[]) {
  return [
    { id: 'all', name: 'All Equipment', count: items.length },
    { id: 'well-control', name: 'Well Control - BOP', count: items.filter((p) => p.category === 'Well Control').length },
    { id: 'drilling', name: 'Drilling Equipment', count: items.filter((p) => p.category === 'Drilling Equipment').length },
    { id: 'production', name: 'Production Equipment', count: items.filter((p) => p.category === 'Production Equipment').length },
    { id: 'completion', name: 'Completion Tools', count: items.filter((p) => p.category === 'Completion Tools').length },
    { id: 'pumps-valves', name: 'Pumps & Valves', count: items.filter((p) => p.category === 'Pumps & Valves').length },
    { id: 'safety', name: 'Safety Equipment', count: items.filter((p) => p.category === 'Safety Equipment').length },
    { id: 'testing', name: 'Testing Equipment', count: items.filter((p) => p.category === 'Testing Equipment').length },
    { id: 'vessels', name: 'Vessels', count: items.filter((p) => p.category === 'Vessels').length },
  ];
}
